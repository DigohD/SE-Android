...
something that could easily be perceived as a problem is that we did not comment code to a conventional extent. We have had a code-then-comment mindset throughout the development, and just want to make a couple of things clear. Commenting code is obviously highly important in group assignments like these, as confusion easily arises when a developer tries to use another persons code. however, in our case this was seen as not needed. Mainly because of the dividing of the project into areas that we tried to stick to, but also since a large majority of the Java code was written during pair programming sessions. When pair programming, there is something way better than a comment line, sitting right beside you: the developer who wrote the code. We've always had the intention to comment and even Javadoc all code prior to submitting.
...
*We will have a finished level where you steer a player ship with a joystick and shoot with a button. There will be enemies coming in waves at random who you can kill and you will get points for killing them. When the level is finished your score will be updated on a working leader board to compare with friends and other players. We will also have a working menu system, sound effects and music.*

The only item that is not implemented today, that is featured in this description, is the button for shooting. After a while, we realized that pressing a button to shoot was mostly annoying and added no value to the game experience. The button was instead exchanged for the power-up inventory slots that we have today. 

We are very satisfied with the resulting product, as it has a lot more features than what we imagined at the start of the course. When we specified our vision (See documentation), we aimed high and really didn't expect to fulfil it, while the MVP was what we thought was the realistic goal. Today we are somewhere in between, perhaps even closer to vision than MVP, and the game is also fairly balanced (A time-consuming process not thought about in the vision).
...
First of all, given that a future project group consists of people working at the same place, meeting schedules should be more regular and consistent than what we have had during this course. Morning meetings almost every day would probably help to organize the project. We also believe that common coding time is beneficial. Even if the different team members are working on different parts, many of the methods in one class might be needed to call another, and a faster communication can ensure that the methods called are up to date and contains the correct types. Communications would probably be different in a future project as well. We have had free around-the-clock communications available at all times. This is probably not the case in a professional working situation. On the other hand, in such an environment people work at regular times. 

The previously discussed issue with a lack of comments in code have not been a problem to us, but in most future projects, commenting code will be the norm. How well a developer can make his code understandable to others is also an indicator of both skill and experience. It is even more important after the product is finished, since future developers might need to modify the software. We are all well aware of this fact.
