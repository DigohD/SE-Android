...
public class DBAdapter {
	...
	/*
	public Cursor getHighestScoreInLevel(int i){
		Cursor c = null, cursor = getHighestLevel();
		c = db.rawQuery("SELECT * FROM highScores WHERE level = '" + i + "'ORDER BY highscore DESC LIMIT 1", null);
		if (c != null) {
			c.moveToFirst();
		}
		return c;
	...
	public Cursor getHighestLevel(){
		Cursor c = db.rawQuery("SELECT * FROM highScores ORDER BY level DESC LIMIT 1", null);
		if (c != null) {
			c.moveToFirst();
		}
		return c;
