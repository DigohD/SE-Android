...
public class HighScoreDataHelper {
	...
	private static final String HIGH_SCORE_TABLE_CREATE =
		"CREATE TABLE " + HIGH_SCORE_TABLE
		+ " (" + KEY_USER + " TEXT, " + KEY_SCORE + " INTEGER, "
		+ KEY_GAME_ID + " INTEGER, " + KEY_DATE + " DATETIME);";
	...
	private static final String INSERT =
		"insert into " + HIGH_SCORE_TABLE
		+ "(" + KEY_USER + "," + KEY_DATE + "," + KEY_SCORE + "," + KEY_GAME_ID + ")"
		+ " values (?,?,?,?)";

	private SQLiteStatement updateStmt;
	private static final String UPDATE =
		"update " + HIGH_SCORE_TABLE
		+ " set " + KEY_DATE + "=?, " + KEY_SCORE + "=?"
	...
	/** Insert a new high score entry
	 * This user doesn't already have a score for this game ID
	 * 
	 * @param entry an entry representing (by player and gameId) the entry to remove
	 * @return the row ID of the row inserted, if this insert is successful. -1 otherwise.
	 */
	public long insert(HighScoreEntry entry) {
		return insert(entry.getUser(), entry.getDate(), entry.getScore(), entry.getGameId());
	}

	/** Update a high score entry
	 * This user already has a score for this game ID, just update the score and date
	 * For raising a particular user's score value for a given game ID
	 * 
	 * @param entry an entry representing (by player and gameId) the entry to remove
	 * @param gameId the identifier of the game
	 */
	public void update(HighScoreEntry entry) {
		update(entry.getUser(), entry.getDate(), entry.getScore(), entry.getGameId());
	}
	...
	// just check if an entry for this player/game combination is already entered.
	public boolean alreadyExists(HighScoreEntry entry) {
		String[] params = { entry.getUser(), String.valueOf(entry.getGameId()) };
		Cursor cursor = this.db.query(HIGH_SCORE_TABLE,
				new String[] { KEY_USER, KEY_GAME_ID },
				KEY_USER + "=? and " + KEY_GAME_ID + "=?", params,
				null, null, null);
	 ...
	 * @param date what date was this high score achieved
	 * @param score what was the score
	 * @param gameId the identifier of the game
	 * @return the row ID of the row inserted, if this insert is successful. -1 otherwise.
	 */
	...
	/** Update an existing high score entry
	 * This user already has a score for this game ID, just update the score and date
	 * For raising a particular user's score value for a given game ID
	 * 
	 * @param name the player's name
	...
	/**
	 * Pull all HighScoreEntries from the DB for a given gameId.
	 * Can also pull all scores if passed ALL_GAMES
	 * 
	 * @param gameId which one (or ALL_GAMES)
	 * @return a list of HighScoreEntries
	 */
	public List<HighScoreEntry> getByGameId(long gameId) {
		String[] params = { String.valueOf(gameId) };
		String condition = KEY_GAME_ID + "=?"; // this is the normal condition for what we're looking
		...
		Cursor cursor = this.db.query(HIGH_SCORE_TABLE,
				new String[] { KEY_USER, KEY_DATE, KEY_SCORE, KEY_GAME_ID },
				condition, params,
				null, null, KEY_GAME_ID + " asc, " + KEY_SCORE + " desc, " + KEY_DATE); // somewhat silly sort
		if (cursor.moveToFirst()) { // do we have anything at all?
			do {
				list.add(entryFromCursor(cursor));  // build the entry and add to the list
	...
	// pull all the relevant information from the cursor to create a HighScoreEntry
	private HighScoreEntry entryFromCursor(Cursor cursor) {
		return new HighScoreEntry(
				cursor.getString(cursor.getColumnIndexOrThrow(KEY_USER)),
