...
public class GameView extends SurfaceView implements SurfaceHolder.Callback {
	...
	// boolean to check if surface has been destroyed which is useful when we
	// want
	// to return to the app after a phone call as example.
	public boolean sdestroyed = false;
	// a timer used for various things such as when and how long to display what
	// level the player is on
	private int timer = 0;
	// timer for the musicplayer, used to ensure the musicplayer only get
	// started once
	private int musicStartTimer;
	// the time we want the first level to take, might be changed in the
	// levelcreator
	...
	// scaling variables used to ensure the game scales properly depending on
	// the phones resolution
	private float scaleX, scaleY;
	// positions for the joystick knob
	private float knobX, knobY;
	// boolean to make sure the musicplayer dont get restarted when it is
	// already on
	...
	// boolean to check wheter a dialogbox is currently showing, useful when we
	// resume the game after for example
	// a phone call
	...
	/**
	 * Updates the gamelogic in the levels and ensures that levels keep coming
	 * continuosly as long as there are levels to generate
	 * 
	 * @param dt
	 *            the timestep, eg the time it takes to complete one frame
	 */
	public void tick(float dt) {
		...
		if (level.isFinished() && GameObjectManager.getPlayer().isLive()) {
			...
			if (timer >= 2 * 60) {
				if (levelID >= Level.getNumOfLevels()) {
					okToRestartMP = false;
					dialogBox("Game completed!", "Highscore: " + GameObjectManager.getPlayer().getScore(), "Restart",
							"Submit score", "Main Menu");
				} else {
					levelID++;
					level.startLevel(levelID);
		...
		if (!GameObjectManager.getPlayer().isLive()) {
			...
			timer++;
			if (timer >= 2 * 60) {
				dialogBox("You died! You made it to level " + (levelID), "Highscore: "
						+ GameObjectManager.getPlayer().getScore(), "Restart", "Submit score", "Main Menu");
				timer = 0;
			}
		}
		if (gwMusicState) {
	...
	/**
	 * Draws everything in the game, like everything in the level, the joystick
	 * and so on
	 * 
	 * @param canvas
	 * @param interpolation
	...
	private void resetGameState() {
		...
		GameObjectManager.getPlayer().getTargetPosition()
				.set(GameObjectManager.getPlayer().getPosition().x, GameObjectManager.getPlayer().getPosition().y);
		okToRestartMP = true;
		displayLevelID = false;
		timer = 0;
		game.resume();
	 ...
	 */
	public void dialogBox(final String title, final String msg, final String positiveBtn, final String negativeBtn) {
	 ...
	 *            the text in the negative button
	 */
	public void dialogBox(final String title, final String msg, final String positiveBtn, final String neutralBtn,
