...
/**
 * Class used for generating sequences of enemies or single enemies to a
 * timeline. The idea is to create a timeline where you can choose to generate
 * sequences to it randomly or manually. There is also an option to add single
 * enemies to a parallell timeline.
 * 
 * @author Anders
 * 
 */
public class EnemyGenerator {
	...
	// the timeline map which has the times that a specific sequence should be
	// generated stored as keys
	// and the specific sequence as value
	private HashMap<Integer, Sequence> sequenceTimeline;
	// the timeline map which has the times that a specific enemy should be
	// generated stored as keys
	 ...
	 * 
	 * @param time
	 *            the time in seconds of the timeline used for sequence/enemy
	 *            generations
	 */
	 ...
	 * 
	 * @param seq
	 *            the sequence to add to the timeline
	 * @param timeStamp
	 *            the time in seconds that the sequence shall appear
	 */
	public void addSequenceToTimeline(Sequence seq, int timeStamp) {
		if (seq.isTimeLimit()) {
			// a min and max time to ensure that the added sequence wont be too
			// close to
	...
	/**
	 * Generates a random timeline of enemy sequences. Timestamps are simply the
	 * specific time in seconds at which a sequence should get generated
	 */
	public void generateRandomTimeLine() {
		// the min and max values are used to avoid generating sequences to
		// close to each other
		// mininum timestamp
		int min = 5;
		// maximum timestamo
		...
		while (i < TIME) {
			...
			// if the there is only one sequences then there is no need to make
			// a new instance
			if (sequences.get(r).getSequences().size() == 1) {
				...
				i += timeStamp;
				// if however there are several sequences to choose from then we
				// must make a new instance
	 ...
	 * map the sequence stored at that time will run. If it matches an entry in
	 * the enemytimeline map the enemy at that time will be initiated and added
	 * to the manager lists, so it can be updated, drawn and checked for
	 * collisions
	 */
	public void tick() {
		if (update) {
